...
int main()
{
	...
	int laco = 1;
	char senhadigitada[10] = "a"; //criação da variável senha
	
	printf("### Cartório da EBAC ###\n\n"); //menu da primeira tela de requisição de senha
	printf("LOGIN DE ADMINISTRADOR\n\n"); //
	printf("Digite a sua senha: ");
	scanf("%s", senhadigitada); //guarda a tentativa de senha de login
	
	if (strcmp(senhadigitada, "admin") == 0) //faz a comparação das senhas, se elas forem iguais retorna = 0
	{
	...
	{
		printf("\nSenha incorreta\n"); //se as senhas forem diferentes, ou seja, retornar algo diferente de 0
	}
}
